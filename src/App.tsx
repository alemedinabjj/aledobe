...
function App() {
    const [areaSelected, setAreaSelected] = useState({} as IArea);

    const [areas, dispatch] = useReducer(
        areasReducer,
        [],
        (initialState) => {
    ...
    return (
        ...
        >
            <button onClick={() => handleRemoveAll()}
                style={{ position: "absolute", top: "0px", left: "0px", zIndex: 999 }}>Remove All</button>
            {areas.map((area) => (
                <Area
                    key={area.id}
                    area={area}
